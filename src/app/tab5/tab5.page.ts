...
export class Tab5Page {
  ...
  async signInGuest() {
    ...
    this.userProfile.PostAnonymousMemberVisitLog(this.businessGroupId, this.businessLocationId, this.sourceId, this.userId).subscribe(async (res: any) => {
      ...
      async (error) => {
        ...
        this.router.navigate(['tab2']);
        const toast = await this.toastCtrl.create({
          message: "To add a new guest, kindly sign out of any existing guest member!!",
          duration: 2500,
          cssClass: 'custom-toast'
        });
        toast.present();
  ...
  async next() {
    if (/^([0-9])\1*$/.test(this.AddNumberDetails.display.value) && this.AddNumberDetails.display.value.length == 10) {
      const toast = await this.toastCtrl.create({
        message: "You're trying to Signin Guest, Click Add Guest for Signin guest member!!",
        duration: 3000,
        cssClass: 'custom-toast'
      });
      toast.present();
      ...
      if (/^[0][0-9]/.test(this.AddNumberDetails.display.value) || this.AddNumberDetails.display.value.length < 10) {
        const toast = await this.toastCtrl.create({
        ...
        this.isLoading = true;
        if (this.multistep.controls['addNumberDetails'].invalid && this.step == 1) {
          return;
        }
          ...
          this.memberProfileService.GetMemberProfileByPhoneNo(this.businessGroupId, this.businessLocationId, this.newMemberPhone).subscribe((resProfile: any) => {
            ...
            this.userProfile.GetMemberBySignout(this.businessLocationId, this.sourceId).subscribe((data: any) => {
              ...
              if (signInId.includes(this.memberProfileExist[0].id)) {
                ...
                this.member.BusinessLocationId = this.businessLocationId;
                ...
                this.userProfile.PostMemberVisitLog(this.member).subscribe(data => {
            ...
            async (error) => {
              if (error.status == 404) {
                ...
                let newMemberProfileData = {
                  ...
                  "lifeTimeVisits": 0,
                  "smsoptIn": false,
                  "emailOptIn": (this.memberDataExist[0].emailId == '' || this.memberDataExist[0].emailId == null
                    || this.memberDataExist[0].emailId == undefined) ? false : true,
  ...
  async submitData() {
    ...
    let newMemberData = {
      ...
      "memberName": (this.AddMemberDetails.name.value == '' || this.AddMemberDetails.name.value == null
        || this.AddMemberDetails.name.value == undefined) ? "USER " + Number(this.AddNumberDetails.display.value).toString().substring(5, 10) : this.AddMemberDetails.name.value,
      ...
      "emailID": (this.AddMemberDetails.email.value == '' || this.AddMemberDetails.email.value == null ||
        this.AddMemberDetails.email.value == undefined) ? null :
      ...
      "memberProfile": [
        {
          ...
          "id": 0,
          "memberId": 0,
          "notes": this.AddMemberDetails.note.value ? this.AddMemberDetails.note.value : null,
          "badgeId": 1,
          "tagId": null,
          "businessGroupId": this.businessGroupId,
          "lastVisitDate": null,
    ...
    if (this.multistep.controls['addMemberDetails'].valid) {
      if (this.isAgeRestriction == true) {
        if ((this.AddMemberDetails.isOverAged.value)?.toString() == 'true') {
          ...
          this.closePopup();
          const toast = await this.toastCtrl.create({
            message: "Please confirm that Member is at least 21 years old to proceed!!",
            duration: 2000,
    ...
    switch (val) {
      case 'delete-left':
        if (this.AddNumberDetails.display.value) {
          ...
          let a = this.AddNumberDetails.display.value.substring(0, displayLength - 1);
          this.multistep.controls['addNumberDetails'].controls['display'].setValue(a);

          let len = this.phoneNumber.length;
          if (len == 11) {
            this.phoneNumber = this.phoneNumber.substr(0, this.phoneNumber.length - 2);
          } else if (len == 7) {
            this.phoneNumber = this.phoneNumber.substr(0, this.phoneNumber.length - 2);
          } else if (len == 5) {
            this.phoneNumber = this.phoneNumber.substr(0, this.phoneNumber.length - 2);
  ...
  addnumber(nbr: string) {
    ...
    if (len >= 10) {
      ...
      this.multistep.controls['addNumberDetails'].controls['display'].setValue(number);
    } else {
