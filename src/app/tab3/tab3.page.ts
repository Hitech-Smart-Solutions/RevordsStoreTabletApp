...
export class Tab3Page {
  ...
  constructor(public activatedRoute: ActivatedRoute, private router: Router, private userProfile: GetUserProfileService,
      ...
      if (params && params["memberData"]) {
        ...
        if (this.memberProfile != undefined && this.memberProfile != null && this.memberProfile != "") {
          ...
          if (this.member.masterMemberID <= 9 && this.member.masterMemberID >= 1) {
            ...
            if (this.member.lifeTimeVisits == 0) {
              this.showBeforeSpinWheelData = true;
              this.myInterval = setInterval(async function () {
                await this.GetSpinWheelText();
              }.bind(this), 1500);
            } else {
              await this.userProfile.GetSpinWheelConfigByMemberIDBusinessGroupID(this.member.memberId, this.businessGroupId).pipe().subscribe({
                next: (data) => {
                  this._defaultOpts = [];
                  ...
                  if (this._defaultOpts[0].configName == "Config 1") {
                    this.showBeforeSpinWheelData = false;
                    this.showSpinWheelData = false;
                    this.myInterval = setInterval(async function () {
                      await this.GetSpinWheelText();
                    }.bind(this), 1500);
                  } else {
    ...
    setInterval(() => {
      if (this.member != undefined && this.member != null && this.member != '' && this.member.length != 0) {
        ...
        this.member.timer = h.toString().padStart(1, '0') + ':' + m.toString().padStart(2, '0') + ':' + s.toString().padStart(2, '0');
      }
    }, 1000);
  }
  ...
  async GetRedeemRewardCount() {
    await this.userProfile.GetRedeemRewardCount(this.businessLocationId, this.sourceId, this.member.memberId).subscribe((data: any) => {
      this.summaryData = data;
      this.CountPromotion = this.CountPromotion + this.summaryData.rewardCount;
      this.EarnedPoints1 = this.summaryData.earnedPoints;
      this.NewTotalPoint = this.summaryData.earnedPoints + this.member.currentPoints;
    },
      async (error) => {
        // console.log(error);
      }
  ...
  async GetPromotionsList() {
    await this.userProfile.GetPromotionsByMemberId(this.businessLocationId, this.member.memberId).subscribe((data: any) => {
  ...
  async GetSpinWheelText() {
    await this.userProfile.GetSpinWheelTextByMemberId(this.businessLocationId, this.member.memberId, CONSTANTS.ISODate()).subscribe((data: any) => {
      this.spinWheelData = data;
      if (this.spinWheelData) {
        this.showSpinWheelData = true;
        this.showBeforeSpinWheelData = false;

        if (this.spinWheelData.points == 0 && this.spinWheelData.isSpinRedeem == false) {
  ...
  async GetAutopilotList() {
    await this.userProfile.GetAutopilotByMemberId(this.businessGroupId, this.member.memberId).subscribe((data: any) => {
  ...
  async GetRewardList() {
    await this.userProfile.GetRewardByMemberId(this.businessGroupId, this.member.memberId).subscribe((data: any) => {
  ...
  async RedeemData() {
    ...
    if (this.promotionData) {
      let objPromotionData = this.promotionData.filter(x => x.stateChecked == true);
    ...
    if (this.autopilotData) {
      let objAutopilotData = this.autopilotData.filter(x => x.stateChecked == true);
    ...
    if (redeemData.autopilotId.length == 0 && redeemData.promotionId.length == 0 && redeemData.rewardId.length == 0
      ...
      this.closePopup();
      setTimeout(() => {
        this.userProfile.UpdateRedeemRewardsInStore(redeemData).subscribe(async (data: any) => {
          this.router.navigate(['tab2']);
          const toast = await this.toastCtrl.create({
  ...
  submit() {
    ...
    setTimeout(() => {
      this.userProfile.UpdateRewardsInStore(redeemData).subscribe((data: any) => {
        this.router.navigate(['tab2']);
  ...
  onPromotionSelected(e: any, p: any) {
    if (e.detail.checked) {
      this.promotionData.filter(x => x.promotionId == p.promotionId)[0].promotionBorder = '2px solid #dda045';
      this.promotionData.filter(x => x.promotionId == p.promotionId)[0].transformScale = 'scale(0.96)';
      this.promotionData.filter(x => x.promotionId == p.promotionId)[0].stateChecked = true;
      this.CountPromotion = this.CountPromotion + 1;
    }
    else if (!e.detail.checked) {
      this.promotionData.filter(x => x.promotionId == p.promotionId)[0].promotionBorder = '2px solid #d9e7ed';
      this.promotionData.filter(x => x.promotionId == p.promotionId)[0].transformScale = 'scale(0.9)';
      this.promotionData.filter(x => x.promotionId == p.promotionId)[0].stateChecked = false;
      this.CountPromotion = this.CountPromotion - 1;
    }
    this.disableRedeemButton();
  }
  ...
  onAutopilotSelected(e: any, p: any) {
    if (e.detail.checked) {
      this.autopilotData.filter(x => x.historyId == p.historyId)[0].autoPilotBorder = '2px solid #2196f396';
      this.autopilotData.filter(x => x.historyId == p.historyId)[0].transformScale = 'scale(0.96)';
      this.autopilotData.filter(x => x.historyId == p.historyId)[0].stateChecked = true;
      this.CountPromotion = this.CountPromotion + 1;
    }
    else if (!e.detail.checked) {
      this.autopilotData.filter(x => x.historyId == p.historyId)[0].autoPilotBorder = '2px solid #d9e7ed';
      this.autopilotData.filter(x => x.historyId == p.historyId)[0].transformScale = 'scale(0.9)';
      this.autopilotData.filter(x => x.historyId == p.historyId)[0].stateChecked = false;
      this.CountPromotion = this.CountPromotion - 1;
    }
    this.disableRedeemButton();
  }
  ...
  GotoProfile() {    
    if (this.step == 1 && !this.isHideRewards && this.member.masterMemberID > 9) {
  ...
  click(val: any) {
    switch (val) {
      case 'delete-left':
        ...
        this.new = this.AddPointDetails.display.value.substr(0, this.new.length - 1);
        if (this.new.length == 0) {
          this.multistep.controls['addPointDetails'].controls['display'].setValue('0');
        } else {
          this.multistep.controls['addPointDetails'].controls['display'].setValue(this.new);
        }
        break;
      case '0':
        this.addnumber('0');
  ...
  addnumber(nbr: string) {
    if (nbr === '0') {
      if (this.newcursor === true) {
        this.multistep.controls['addPointDetails'].controls['display'].setValue(nbr);
        this.newcursor = false;
      }
      else if (this.multistep.controls['addPointDetails'].controls['display'].value !== '0') {
        this.multistep.controls['addPointDetails'].controls['display'].setValue(this.multistep.controls['addPointDetails'].controls['display'].value.toString() + nbr);
      }
    }
    else {
      if (this.newcursor === true) {
  ...
  disableRedeemButton() {
    ...
    if (redeemData.autopilotId.length == 0 && redeemData.promotionId.length == 0 && redeemData.rewardId.length == 0
      && ((this.spinWheelData != null && this.spinWheelData.points != 0) || redeemData.spinWheelId.length == 0)) {
      this.disableRedeemBtn = true;
    }
    else {
      this.disableRedeemBtn = false;
